...
class GetDistrictApiView(APIView):
    ...
    def post(request):
        try:
            ...
            district = LookupDistrict.objects.filter(municipality=municipality_id, is_active=1).order_by("district_name").values('id', 'district_name')
            return Response(response.parsejson("Fetch Data.", district, status=201))
        except Exception as exp:
            return Response(response.parsejson(str(exp), exp, status=403))
...
class GetMunicipalityApiView(APIView):
    ...
    def post(request):
        try:
            ...
            municipality = LookupMunicipality.objects.filter(state=state_id, is_active=1).order_by("municipality_name").values('id', 'municipality_name')
