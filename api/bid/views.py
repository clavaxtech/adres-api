    ...
    def post(request):
        try:
            ...
            if "user_id" in data and data['user_id'] != "":
                ...
                if user is None:
                    agent_data = Users.objects.filter(id=user_id, status=1, user_type=2).first()
            ...
            site_id = Users.objects.filter(id=user_id, status=1).first()
            site_id = int(site_id.site_id)
            ...
            if site_id:
                ...
                bid_registration = BidRegistration.objects.filter(Q(domain=site_id) & Q(status=1) & Q(property__agent=user_id)).exclude(property__status__in=[5])
            # -------Filter-------
    ...
    def post(request):
        try:
            ...
            if user_type == 2:
                ...
                bid_registration = BidRegistration.objects.get(Q(id=registration_id) & Q(domain=site_id) & Q(property__agent=user_id))
            ...
            with transaction.atomic():
                try:
                    ...
                    if user_type == 2:
                        ...
                        bid_registration = BidRegistration.objects.filter(Q(id=registration_id) & Q(domain=domain) & Q(property__agent=user_id)).first()
                    if bid_registration is None:
...
class SubdomainBidHistoryView(APIView):
    ...
    def post(request):
        try:
            ...
            new_bid = Bid.objects.filter(Q(property=property_id) & Q(is_canceled=0))
            if register_user is not None:
...
class SendEnglishAuctionEmailView(APIView):
    ...
    def post(request):
        try:
            ...
            extra_data["user_name"] = backup_user.user.first_name + ' ' + backup_user.user.last_name
