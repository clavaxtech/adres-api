...
class PropertyListingApiView(APIView):
    ...
    def post(request):
        try:
            ...
            if is_agent is not None and users.user_type_id == 2: # ------------For Developer-----------
                property_listing = property_listing.filter(developer=user_id)
            elif is_agent is not None: # ---------For Employee--------
            ...
            if "developer_id" in data and data["developer_id"] != "":
                ...
                property_listing = property_listing.filter(Q(developer=developer_id))
...
class SellerDashboardPropertyListingApiView(APIView):
    ...
    def post(request):
        try:
            ...
            property_listing = PropertyListing.objects.filter(agent=agent_id)
...
class FrontPropertyListingApiView(APIView):
    ...
    def post(request):
        try:
            ...
            property_listing = PropertyListing.objects.filter(domain=site_id, seller_status=27, status=status)
        ...
        try:
            ...
            if favourite_property is None:
                ...
                if serializer.is_valid():
                    ...
                    content = "Listing added to favorites! <span>[" + data.property.property_name + "]</span>"
                else:
                    copy_errors = serializer.errors.copy()
                    return Response(response.parsejson(copy_errors, "", status=403))
            else:
                ...
                content = "Listing removed from favorites! <span>[" + favourite_property.property.property_name + "]</span>"
...
class PropertyAuctionDashboardApiView(APIView):
    ...
    def post(request):
        try:
            ...
            if "user_id" in data and data['user_id'] != "":
                ...
                if is_super_admin is None:
                    ...
                    if users is None:
                        ...
                        # if users is None:
                        #     is_agent = True
                        #     users = Users.objects.filter(id=user_id, network_user__domain=domain_id, network_user__is_agent=1, network_user__status=1, status=1, user_type=4).first()
                        #     if users is None:
                        #         return Response(response.parsejson("User not exist.", "", status=403))
                        if users is None:
                            return Response(response.parsejson("User not exist.", "", status=403))
            ...
            if is_agent is not None and users.user_type_id == 2:
                property_data = property_data.filter(developer=user_id)
            elif is_agent is not None:
                property_data = property_data.filter(agent=user_id)      
    ...
    def post(request):
        try:
            ...
            try:
                ...
                image_url = web_url+'/static/images/property-default-img.png'
                if upload is not None:
                    image = upload.upload.doc_file_name
                    bucket_name = upload.upload.bucket_name
                    image_url = settings.AZURE_BLOB_URL+bucket_name+'/'+image
                if is_approved and int(is_approved) == 28:
                    domain_url = settings.REACT_FRONT_URL+"/my-properties"
                elif is_approved and int(is_approved) == 29:
                    domain_url = settings.REACT_FRONT_URL+"/notification/"+str(property_id)
                else:
                    domain_url = settings.REACT_FRONT_URL+"/my-properties"
                property_address = property_detail.address_one
            ...
            try:
                ...
                # check who approved/not approved
                if is_approved and int(is_approved) == 28:
                    content = "Your listing is ready for publish! <span>[" + prop_name + "]</span>"
                elif is_approved and int(is_approved) == 29:
                    content = "Your listing is returned for update! <span>[" + prop_name + "]</span>"
                elif is_approved and int(is_approved) == 24:
                    content = "Your listing is under review! <span>[" + prop_name + "]</span>"
                if property_detail.agent_id != user_id:
                    # send notif to agent person
                    add_notification(
                        site_id,
                        "Listing Approval",
                        content,
...
class AddListingApiView(APIView):
    ...
    def post(request):
        try:
            ...
            if "user_id" in data and data['user_id'] != "":
                user_id = int(data['user_id'])
                users = Users.objects.filter(id=user_id, user_type__in=[2, 5], status=1).first()
                if users is None:
                        return Response(response.parsejson("User not exist.", "", status=403))
                creater_user_type = users.user_type_id
            else:
            ...
            if step == 1:
                ...
                if "tags" in data and type(data['tags']) == list and len(data['tags']) > 0:
                    ...
                    return Response(response.parsejson("tags is required.", "", status=403))    
